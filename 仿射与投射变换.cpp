...
int main(int argc, char** argv)
{
	...
	*/
	...
	//@透视变换
	//将二维的图弄得看起来像三维 ，近大远小， 游戏劣质贴图上线（
	/*cv::Point2f srcQuad[] = {cv::Point2f(0,0) //源点
							,cv::Point2f(imgSrc.cols-1,0)
							,cv::Point2f(imgSrc.cols-1, imgSrc.rows-1)
							,cv::Point2f(0,imgSrc.rows-1)};

	cv::Point2f dstQuad[] = {cv::Point2f(imgSrc.cols*0.05f,imgSrc.rows*0.33f) //目标点
							,cv::Point2f(imgSrc.cols*0.9f,imgSrc.rows*0.25f)
							,cv::Point2f(imgSrc.cols*0.8f,imgSrc.rows*0.9f)
							,cv::Point2f(imgSrc.cols*0.2f,imgSrc.rows*0.7f)};

	cv::Mat mo = cv::getPerspectiveTransform(srcQuad,dstQuad);//计算透视矩阵 3x3
	cv::warpPerspective(imgSrc,imgDst,mo,imgSrc.size(),cv::INTER_LINEAR,cv::BORDER_CONSTANT,cv::Scalar());

	for (int i = 0; i < 4; i++) {
		cv::circle(imgDst,dstQuad[i],5,cv::Scalar(255,0,255),-1,8,0);
	}*/
